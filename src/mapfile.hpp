...
public:
    ...
    void* MapData(off_t offset, size_t length)
    {
        if((src = mmap(0, length, PROT_READ, MAP_SHARED, fdin, offset)) == (void *) -1)
        {
            perror("mmap file error");
            exit(-1);
        }
    ...
    int UnMapData( void * addr , size_t length)
    {
        int RetVal;
        if((RetVal = munmap(addr, length)) == (int) -1)
        {
            perror("munmap file error");
            exit(-1);
        }
        return RetVal;
    }
